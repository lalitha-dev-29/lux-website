...
- **Storage** — one table, `journal_posts` (see `supabase/schema.sql` for the full schema). The columns the app actually reads and writes are `title`, `slug`, `excerpt`, `external_url`, `status`, `published_at`, `created_at`, `updated_at`. The table also still *has* `content`, `category`, `tags` and `cover_image` from the initial design; the editor no longer offers them and every save writes them back as `NULL`, so they're inert. They're left in place rather than dropped so no existing row is destroyed — drop them with an `ALTER TABLE` if you're sure you'll never want them.
...
- **The editor authors three fields: title, excerpt, and LinkedIn URL** — matching the original Markdown frontmatter exactly. Every Journal entry is a link-preview card pointing at a LinkedIn post; there is no on-site body copy, so there is no rich-text editor, no Markdown parsing, and correspondingly no HTML-sanitisation surface to get wrong.
...
All three fields — **Title**, **Excerpt** and **LinkedIn Post URL** — are required. The URL requirement is also backed by the database's `journal_posts_has_body` constraint, so it can't be skipped via a raw API call either. The public card behaves exactly like the original design: "View on LinkedIn", opening in a new tab.

Note this means a post can't be drafted before its LinkedIn URL exists. If you'd rather write the excerpt first and add the link later, run `alter table public.journal_posts drop constraint journal_posts_has_body;` in the SQL Editor and change `validateJournalForm()` in `src/lib/journalForm.ts` to stop requiring the URL.
...
- **Content injection**: admin-authored text is HTML-escaped before insertion into the DOM — there is no code path that interprets it as markup. All string-built markup goes through the single `escapeHtml`/`safeUrl` pair in `src/lib/html.ts`; `safeUrl` additionally refuses `javascript:`/`data:` hrefs, so a URL field can never become a script trigger even as more roles are added later.
