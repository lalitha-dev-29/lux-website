...
- **Content injection**: plain-text content is HTML-escaped before insertion into the DOM (see "Content is plain text" above) — there is no code path that interprets admin-authored text as markup. All string-built markup goes through the single `escapeHtml`/`safeUrl` pair in `src/lib/html.ts`; `safeUrl` additionally refuses `javascript:`/`data:` hrefs, so a URL field can never become a script trigger even as more roles are added later.
